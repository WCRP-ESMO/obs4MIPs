...
// Configuration
const CONFIG = {
  repoUrl: 'https://github.com/WCRP-ESMO/obs4MIPs',
  repoName: 'WCRP-ESMO/obs4MIPs',
  customLinks: [],
  // urlPrefix: 'guidance'
};
...
function setupCollapsibleNav() {
  ...
  groups.forEach((group, index) => {
    ...
    if (!label.dataset.collapsibleSetup) {
      label.classList.add('expanded'); // changed from remove
      if (hasActivePage) {
  ...
  if (!document.querySelector('.footer-attribution')) {
    ...
    if (docsDiv) {
      ...
      attribution.innerHTML = `
        <p>
          Built with mkdocs using the <a href="https://github.com/asiffer/mkdocs-shadcn">shadcn</a> theme
          for <a href="https://wcrp-esmo.org">WCRP-ESMO</a>.
        </p>
      `;
